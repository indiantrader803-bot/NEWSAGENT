@echo off
REM ═══════════════════════════════════════════════════════════════════════
REM Push All 24/7 Market Bot Changes to GitHub
REM ═══════════════════════════════════════════════════════════════════════
...
if errorlevel 1 (
    ...
    git init
    git remote add origin https://github.com/YOUR_USERNAME/Forexsignal.git
    echo [INFO] Please update the remote URL above with your GitHub repo
    pause
    exit /b 1
)
...
echo [INFO] Creating commit with comprehensive message...
...
if errorlevel 1 (
    ...
    echo This is normal if all changes were already committed
)
...
if errorlevel 1 (
    ...
    echo 1. Not logged into GitHub (run: git config credential.helper manager)
    echo 2. Remote URL incorrect (run: git remote -v)
